...
async fn get_or_initialize_pool(database_url: &Option<String>) -> anyhow::Result<PgPool> {
    ...
    let url = database_url.clone().unwrap_or_else(|| {
        std::env::var("KORVUS_DATABASE_URL").unwrap_or_else(|_|
            std::env::var("DATABASE_URL").expect("Please set KORVUS_DATABASE_URL environment variable or explicitly pass a database connection string to your collection"))
    });
    if let Some(pool) = pools.get(&url) {
        Ok(pool.clone())
    } else {
        let acquire_timeout = std::env::var("KORVUS_CHECKOUT_TIMEOUT")
            .ok()
            .map(|v| v.parse::<u64>())
            .transpose()
            .context("Error parsing KORVUS_CHECKOUT_TIMEOUT, expected an integer")?
            .map(anyhow::Ok)
            .unwrap_or_else(|| {
                Ok(std::env::var("KORVUS_POOL_ACQUIRE_TIMEOUT")
                    .ok()
                    .map(|v| v.parse::<u64>())
                    .transpose()
                    .context("Error parsing KORVUS_POOL_ACQUIRE_TIMEOUT, expected an integer")?
                    .unwrap_or(30000))
            })?;
        let acquire_timeout = Duration::from_millis(acquire_timeout);

        let max_lifetime = std::env::var("KORVUS_POOL_MAX_LIFETIME")
            .ok()
            .map(|v| {
                anyhow::Ok(Duration::from_millis(v.parse::<u64>().context(
                    "Error parsing KORVUS_POOL_MAX_LIFETIME, expected an integer",
                )?))
            })
            .transpose()?;

        let idle_timeout = std::env::var("KORVUS_POOL_IDLE_TIMEOUT")
            .ok()
            .map(|v| {
                anyhow::Ok(Duration::from_millis(v.parse::<u64>().context(
                    "Error parsing KORVUS_POOL_IDLE_TIMEOUT, expected an integer",
                )?))
            })
            .transpose()?;

        let max_connections = std::env::var("KORVUS_POOL_MAX_CONNECTIONS")
            .ok()
            .map(|v| v.parse::<u32>())
            .transpose()
            .context("Error parsing KORVUS_POOL_MAX_CONNECTIONS, expected an integer")?
            .unwrap_or(10);

        let min_connections = std::env::var("KORVUS_POOL_MIN_CONNECTIONS")
            .ok()
            .map(|v| v.parse::<u32>())
            .transpose()
            .context("Error parsing KORVUS_POOL_MIN_CONNECTIONS, expected an integer")?
            .unwrap_or(0);
...
#[cfg(feature = "python")]
#[pyo3::pymodule]
fn korvus(_py: pyo3::Python, m: &pyo3::types::PyModule) -> pyo3::PyResult<()> {
