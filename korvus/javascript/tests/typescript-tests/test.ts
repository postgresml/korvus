...
it("can create collection", () => {
  let collection = korvus.newCollection("test_j_c_ccc_0");
  expect(collection).toBeTruthy();
});

it("can create model", () => {
  let model = korvus.newModel("test", "openai", {
...
it("can create splitter", () => {
  let splitter = korvus.newSplitter();
  expect(splitter).toBeTruthy();
});

it("can create pipeline", () => {
  let pipeline = korvus.newPipeline("test_j_p_ccp");
  expect(pipeline).toBeTruthy();
});

it("can create single field pipeline", () => {
  let model = korvus.newModel();
  let splitter = korvus.newSplitter();
  let pipeline = korvus.newSingleFieldPipeline("test_j_p_ccsfp", model, splitter);
  expect(pipeline).toBeTruthy();
});

it("can create builtins", () => {
  let builtins = korvus.newBuiltins();
...
it("can search", async () => {
  let pipeline = korvus.newPipeline("test_j_p_cs", {
  ...
  let collection = korvus.newCollection("test_j_c_tsc_15")
...
it("can vector search", async () => {
  ...
  let collection = korvus.newCollection("test_j_c_cvs_4")
  await collection.add_pipeline(pipeline)
  await collection.upsert_documents(generate_dummy_documents(5))
...
it("can vector search with query builder", async () => {
  let model = korvus.newModel("intfloat/e5-small-v2", "pgml", { prompt: "passage: " });
  let splitter = korvus.newSplitter();
  let pipeline = korvus.newSingleFieldPipeline("0", model, splitter);
  let collection = korvus.newCollection("test_j_c_cvswqb_2");
...
it("can rag", async () => {
  let pipeline = korvus.newPipeline("0", {
    body: {
      splitter: { model: "recursive_character" },
      semantic_search: {
  ...
  let collection = korvus.newCollection("test_j_c_cr_0")
  await collection.add_pipeline(pipeline)
  await collection.upsert_documents(generate_dummy_documents(5))
...
it("can upsert and get documents", async () => {
  let collection = korvus.newCollection("test_p_c_cuagd_1");
...
it("can delete documents", async () => {
  let collection = korvus.newCollection("test_p_c_cdd_2");
  await collection.upsert_documents(generate_dummy_documents(3));
...
it("can order documents", async () => {
  let collection = korvus.newCollection("test_j_c_cod_0");
  await collection.upsert_documents(generate_dummy_documents(3));
  let documents = await collection.get_documents({
    order_by: {
      id: "desc",
...
it("can transformer pipeline", async () => {
  const t = korvus.newTransformerPipeline("text-generation", "meta-llama/Meta-Llama-3-8B-Instruct");
